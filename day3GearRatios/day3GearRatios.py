...
def locate_part_number(new_r, new_c, grid):
    l = new_c
    r = new_c
...
def append_part_numbers(r, c, grid, part_numbers):
    ...
    for move in possible_moves:
        ...
        if grid[new_r][new_c].isnumeric():
            part_numbers.append(locate_part_number(new_r, new_c, grid))

part_numbers = []
for r in range(N):
    for c in range(M):
        # if we hit a symbol
        if grid[r][c] != '.' and not grid[r][c].isnumeric():
            append_part_numbers(r, c, grid, part_numbers)
